            ...
            <div class="row gy-4">
                @forelse($category->posts as $post)
                    <div class="col-xl-3 col-md-6 mb-3" style="height: auto">
                        <article>
                        <div class="post-img">
                            <img src="{{ env('APP_URL') . Storage::url($post->thumbnail)  }}" alt="" class="img-fluid">
                        </div>

                        <p class="post-category">
                            <span class="badge bg-success"> {{ $category->category_name }} </span> | {{ ApplicationHelper::getFormatTime($post->uploaded_at) }}
                        </p>
                ...
                @endforelse
                {{ $category->posts->links('pagination::bootstrap-4') }}
            </div>
        </div>
